#!/usr/bin/env bash
# Scrubbed PD-OS + portfolio export safe for public GitHub (meerkat-commits).
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
DEST="${1:-$REPO_ROOT/portfolio/.github-publish}"
...
# Portfolio dev handoff + Figma extract + prior export artifacts
rm -f "$DEST/portfolio/THREAD-HANDOFF.md"
rm -rf "$DEST/portfolio/.fig-extract"
rm -rf "$DEST/portfolio/.public-export"
rm -rf "$DEST/portfolio/.github-publish"
